...
select distinct m.firstname || ' ' || m.surname as name, f.name  as facility from 
members as m
...
select memname, name, cost
from
(select  m.firstname || ' ' || m.surname as memname, f.name,
case when m.memid = 0 then b.slots * f.guestcost
else b.slots * f.membercost end as Cost
from members m
left join bookings b 
...
on f.facid = b.facid
where
b.starttime >= '2012-09-14 00:00:00' and b.starttime < '2012-09-15 00:00:00') as sub
where cost > 30
order by cost desc
...
select distinct m.firstname || ' ' || m.surname as memname, f.name
from members m
left join bookings b 
on b.memid = m.memid
left join facilities f
...
select distinct f.name, extract(month from b.starttime) as month
from members m
left join bookings b 
on b.memid = m.memid
left join facilities f
